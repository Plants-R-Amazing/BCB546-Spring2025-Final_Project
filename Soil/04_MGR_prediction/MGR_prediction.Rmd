...
data<- read.csv("./data/masterTable.csv", row.names =1)
...
write.csv(data_CorspearmanAdj, "./results/SupTable2_Correlations_MGR_soilParameters.csv")
...
masterTable <- read.csv("./data/masterTable.csv", row.names = 1, header = T) %>% select_if(~ !any(is.na(.)))  
...
write.csv(melted_cormat, "./results/Correlation_matrix.csv")
...
ggsave("./plots/SupFig2_Correlationmatrix.jpg",
       plot = p_heatmap,
       width = 30,
       height = 30,
       units = "cm",
...
write.csv(selection,"./results/masterTable_pre-selection_soil.csv")
...
masterTable <- read.csv("./results/masterTable_pre-selection_soil.csv", row.names = 1, header = T)
...
write.csv(importantVariables,  "./results/SupTable4_RandomForest_MGR_soil.csv")
...
data <- read.csv("./masterTable_pre-selection_soil.csv", row.names =1)
...
soil <- read.csv("./data/masterTable.csv", row.names =1) %>%
...
svg("./plots/Fig2_PCA_soil_parameters.svg",
  width = 4.5,
  height = 4.5,
  )
...
data <- read.csv("./data/microbiome_soil_sampleData.csv")
...
#otu data
otus <- read.csv("./data/microbiome_soil_otus.csv", row.names = 1)
phyloseq
OTU = otu_table(otus, taxa_are_rows = TRUE)

#taxonomy data
taxonomy <- read.table("./data/microbiome_soil_taxonomy.tab", sep="\t", row.names = 1)
...
write.csv(OTU_new, "./results/SupTable6_OTU_table_soil_merged_rarefied.csv", row.names = T)
write_phyloseq(rarefiedMergedReplicates, 'TAXONOMY')


jpeg("./plots/RarefactionCurves_soil.jpg", width =1000, height = 800, res = 200)
...
otus2 <- read.csv("./results/SupTable6_OTU_table_soil_merged_rarefied.csv", row.names = 1)
totus <- as.data.frame(t(otus2))
OTU2 = otu_table(totus, taxa_are_rows = TRUE)

samples2 <- read.csv("./data/masterTable.csv") %>%
...
ggsave("./plots/Soil_microbiome_phylum.jpg",
  plot = p_all2,
  width = 50,
  height = 25,
   units = "cm"
...
MGR <- read.csv("./data/masterTable.csv", row.names =1) %>%
       dplyr::select(MGR) %>%
       drop_na()
...
colnames(data_CorspearmanAdj) <- c("r", "p")
write.csv(data_CorspearmanAdj, "./results/Correlations_MGR_soilOTUs.csv")
...
sink(file="./results/IndicSpecies_MGR_cat.txt")
...
taxonomy <- read.table("./data/microbiome_soil_taxonomy.tab", sep="\t")
...
write.csv(indic_taxa, "./results/IndicSpecies_MGR.csv")

#add abundance values
abundance <- read.csv("./results/SupTable6_OTU_table_soil_merged_rarefied.csv")
...
write.csv(MGR_taxa_abundance, "./results/SupTable7_IndicSpecies_MGR.csv")
...
write.csv(sigtab, "./results/SupTable8_Deseq_soil_MGR.csv", row.names = T)
...
ggsave("./plots/Deseq_soil_MGR.jpg",
  plot = p_deseq_soil
  )
...
soil <- read.csv("./data/masterTable.csv", row.names = 1, header = T) %>%
              dplyr::select(MGR) %>%
              drop_na()
...
write.csv(importantVariables,  "./results/SupTable9_RandomForest_MGR_soilOTUs.csv")
...
otus <- read.csv("./results/OTU_table_soil_merged_rarefied.csv", row.names =1) %>%
...
write.csv(predictors, "./results/masterTable_pre-selection_soil_otus_high.csv")


data <- read.csv("./results/masterTable_pre-selection_soil_otus_high.csv", row.names =1)
...
write.csv(predictors, "./results/masterTable_pre-selection_soil_otus_low.csv")


data <- read.csv("./results/masterTable_pre-selection_soil_otus_low.csv", row.names =1)
model <- glmulti(MGR ~., data=data, method="h",crit="aicc",fitfunction = "glm", level =1, maxsize = 10)
...
soilOTUpredictors <- read.csv("./results/SupTable6_OTU_table_soil_merged_rarefied.csv", row.names =1) %>%
             dplyr::select(OTU16,OTU18,OTU49,OTU177,OTU251,
                           OTU388,OTU561,OTU58,OTU68,OTU142,
                           OTU227,OTU273,OTU392)

write.csv(soilOTUpredictors, "./results/soilOTUpredictors.csv")
```
...
otu_table  <- read.csv("./results/SupTable6_OTU_table_soil_merged_rarefied.csv", header=T, sep=",", check.names = F)

colnames(otu_table) <- paste("s", colnames(otu_table), sep = "")

colnames(otu_table)[1] <- "Field"

masterTable <- read.csv("./data/masterTable.csv") %>%
               select_if(~ !any(is.na(.)))  
...
jpeg("./plots/SupFig3_PCoA_microbiome_soil_Year.jpg",
  width = 20,
  height = 20,
  units = "cm",
  res = 800
...
svg("./plots/Fig3_pRDA_soil_microbiome.svg",
  width = 4.5,
  height = 4.5)
...
otus <- read.csv("./results/SupTable6_OTU_table_soil_merged_rarefied.csv")
...
write.csv(soil_indicators, "./results/SummedHighMGR_soilIndicators.csv")
...
ggsave("./plots/Fig4_Scatter_summedOTUs_high.svg",
...
ggsave("./plots/Fig4_Scatter_summedOTUs_low.svg",
...
ggsave("./plots/Fig4_Distribution_highMGROTUs.svg",
  plot = p_highMGROTUs,
  width = 20,
  height = 12,
  units = "cm"
...
ggsave("./plots/SupFig4_Distriubtion_lowMGROTUs.jpg",
  plot = p_lowMGROTUs,
  width = 50,
  height = 25,
  units = "cm",
...
colonization_data <- read.csv("./data/totalColonization.csv")
...
write.csv(totalColonization, "./results/totalColonization.csv", row.names = F)
...
#write output
write.csv(delta_totalColonization, "./results/delta_totalColonization.csv", row.names = F)
...
#sample data
samples  <- read.csv("./data/microbiome_roots.csv", header=T, sep=",", check.names = F, row.names = 1)
SAM = sample_data(samples)

#otu data
otus <- read.csv("./data/microbiome_roots_otus.csv",row.names = 1)
OTU = otu_table(otus, taxa_are_rows = TRUE)

#taxonomy data
taxonomy <- read.table("./data/microbiome_root_taxonomy_mod.txt", sep="\t", row.names = 1)
colnames(taxonomy) <- c("Domain", "Phylum", "Class", "Order", "Family", "Genus", "Species")
taxonomy <- as.matrix(taxonomy)
TAX = tax_table(taxonomy)
...
write.csv(OTU, "./results/OTU_table_root_rarefied.csv", row.names = T)
...
ggsave("./plots/SupFig6_RelativeAbundance_roots_phylum.jpg",
  plot = p_all2,
  width = 50,
  height = 25,
   units = "cm"
...
jpeg("./plots/SupFig7_RarefactionCurves_root.jpg", width =1000, height = 800, res = 200)
...
data  <- read.csv("./data/microbiome_roots.csv", header=T, sep=",", check.names = F, row.names = 1) 
...
write.csv(OTU_Genus, "./results/OTU_table_root_Genus_level.csv", row.names = T)
...
write.csv(OTU_Genus_top15, "./results/OTU_table_root_Genus_level_top15.csv", row.names = T)
...
write.csv(OTU_Genus_rel, "./results/OTU_table_root_Genus_top15_rel.csv", row.names = T)
...
svglite("./plots/SupFig10_Root_genus_2020.svg",
        width = 8,
        height = 5)
plot(p_all2)
dev.off()
...
data  <- read.csv("./data/microbiome_roots.csv", header=T, sep=",", check.names = F) %>%
...
jpeg("./plots/SupFig8_Ordination_root_year_MGR.jpg",
  width = 30,
  height = 15,
  units = "cm",
  res = 600
...
jpeg("./plots/SupFig9_PCoA_microbiome_root_treatment.jpg",
  width = 20,
  height = 20,
  units = "cm",
  res = 600
...
SAF22_RA <- read.csv("./data/SAF22abundance.csv") %>%
            dplyr::select(Field, delta_rel) 


totalColonization <- read.csv("./results/delta_totalColonization.csv") %>%
...
svg("./plots/Fig3_establishment.svg",
     width = 12, height=6)
...
totCol <- ggscatter(SAF22_totalColonization_MGR,
                    ...
                    xlab="Δ Total root colonisation [%]",
                    ylab="MGR [%]") +
...
SAF22_totCol <- ggscatter(SAF22_totalColonization_MGR, 
                          ...
                          add.params = list(color = "blue"),
                          xlab="Δ SAF22 rOTUs [%]",
                          ylab="Δ Total root colonisation [%]") +
...
ggsave("./plots/SupFig5_MGR_AMF_establishment.jpg",
...
write.csv(predictors, "./results/final_predictors_for_model_input.csv")
...
data <- read.csv("./results/final_predictors_for_model_input.csv", row.names =1)
...
relaimpo1 <- read.csv("./data/RelativeImportance_FinalModel1.csv") %>%
  mutate(Predictors =recode(Predictors, "Total colonisation (microscopy)"="Δ Total root colonisation (micropscopy)")) %>%
  mutate(Predictors =recode(Predictors, "Relative abundace SAF22 (root OTUs)"="Δ Relative abundance SAF22 (root OTUs)")) 

#relative importance (calc.relaimpo(model2)) and significance (summary(model2)) added in excel
relaimpo2 <- read.csv("./data/RelativeImportance_FinalModel2.csv")

#relative importance (calc.relaimpo(model3)) and significance (summary(model3)) added in excel
relaimpo3 <- read.csv("./data/RelativeImportance_FinalModel3.csv")
...
MGR <- read.csv("./data/masterTable.csv", header = T) %>%
              dplyr::select(Field, MGR) %>%
              drop_na()
...
ggsave("./plots/SupFig12_pathogens.jpg",
...
otu18 <- read.csv("./results/SupTable6_OTU_table_soil_merged_rarefied.csv") %>%
  rename("Field" = X) %>%
  dplyr::select(Field, OTU18)

soil <- read.csv("./data/masterTable.csv")
...
colnames(data_CorspearmanAdj) <- c("r", "p")
write.csv(data_CorspearmanAdj, "./results/Correlations_OTU18_soilParameters.csv")
...
ggsave("./plots/SupFig11_Scatter_plots_OTU18.jpg",
...
otu58 <- read.csv("./results/SupTable6_OTU_table_soil_merged_rarefied.csv") %>%
  rename("Field" = X) %>%
...
data_CorspearmanAdj <- cbind(t(spearman$r), t(spearman$p))
colnames(data_CorspearmanAdj) <- c("r", "p")
write.csv(data_CorspearmanAdj, "./results/Correlations_OTU58_soilParameters.csv")
...
ggsave("./plots/SupFig13_Scatter_plots_OTU58.jpg",
...
write.csv(sigtab, "./results/Deseq_root_high_MGR.csv", row.names = T)
...
sigtab <- read.csv("./data/Deseq_root_high_MGR_guild.csv", row.names=1)
...
write.csv(sigtab, "./results/Deseq_root_low_MGR.csv", row.names = T)
```
...
sigtab <- read.csv("./data/Deseq_root_low_MGR_guild.csv", row.names=1)
...
data <-read.csv("./data/biomass2018.csv") %>%
