...
#                   multithread=threads_to_use, nbases=1e8)

# 4. Reduce BAND_SIZE from 32 to 16
# dd <- dada(drp, err=err, BAND_SIZE=16, multithread=threads_to_use)
...
# start_time <- Sys.time()
# err <- learnErrors(drp, errorEstimationFunction=PacBioErrfun, BAND_SIZE=16, multithread=threads_to_use, nbases=1e8)
# end_time <- Sys.time()
...
#### CHANGE THIS PATHS ####
source <- "/lab/biohpc/students/sadiya/r_analysis_proj_soil_2"
path.in <- "/lab/biohpc/students/sadiya/r_analysis_proj_soil_2/01_demultiplex/out/" #needs a "/" at the end
path.primer <- "/lab/biohpc/students/sadiya/r_analysis_proj_soil_2/01_demultiplex/" #needs a "/" at the end
path.rds <- "RDS/"
path.out <- "output/"
taxa_database <- "/lab/biohpc/students/sadiya/r_analysis_proj_soil_2/utax_reference_dataset_10.05.2021.fasta"
...
threads_to_use <- floor(total_cores * 0.75)  # Use 75% of cores, rounded down
print(paste("System has", total_cores, "cores. Using", threads_to_use, "cores (75%) for parallel operations"))
...
for (run in runs) {
  ...
  #get primers
  print("Using primer.fasta file for root sample")
  ...
  primerF <- c(primers[["primerF1"]][1], primers[["primerF2"]][1])
  primerR <- c(primers[["primerR1"]][1], primers[["primerR2"]][1])  

  ##remove primers
  #print("Removing primers...")
  #primer_start_time <- Sys.time()
  ...
  #primer_end_time <- Sys.time()
  #print(paste("Primer removal completed in:", difftime(primer_end_time, primer_start_time, units="mins"), "minutes"))

  # First create the noprimers directory
  dir.create(file.path(path_run, "noprimers"), showWarnings=FALSE, recursive=TRUE)
  
  # Define final output file paths
  nop <- paste0(path_run, "/noprimers/", basename(fns))
  
  # Initial file paths
  current_fns <- fns
  
  # Loop through each primer pair with error handling
  for (i in 1:length(primerF)) {
    # Create temporary file paths for this iteration
    temp_nop <- paste0(path_run, "/noprimers/temp_", i, "_", basename(current_fns))
    
    # Try to remove primers, with error handling
    tryCatch({
      print(paste("Attempting primer removal with primer pair", i))
      print(paste("Forward primer:", primerF[i]))
      print(paste("Reverse primer:", dada2::rc(primerR[i])))
      
      # Check if input files exist
      if (!all(file.exists(current_fns))) {
        missing_files <- current_fns[!file.exists(current_fns)]
        print(paste("Warning: The following input files don't exist:", 
                    paste(missing_files, collapse=", ")))
      }
      
      # Remove this set of primers
      prim <- removePrimers(current_fns, temp_nop,
                           primer.fwd = primerF[i],
                           primer.rev = dada2::rc(primerR[i]),
                           orient = TRUE, verbose = TRUE)
      
      # Check if any reads passed primer detection
      if (sum(prim[,2]) == 0) {
        print("No reads passed primer detection. Using original files for next iteration.")
        # If no reads passed, keep using the original files
        # Copy or symlink the original files to the temporary location
        file.copy(current_fns, temp_nop, overwrite = TRUE)
      }
      
      # Update the input files for the next iteration
      current_fns <- temp_nop
      
    }, error = function(e) {
      # Handle errors gracefully
      print(paste("Error in primer removal for pair", i, ":", e$message))
      print("Using original files for next iteration.")
      
      # If error occurs, copy original files to temporary location for next iteration
      file.copy(current_fns, temp_nop, overwrite = TRUE)
      current_fns <- temp_nop
    })
  }
  
  # Rename the final files to the desired output names
  file.rename(current_fns, nop)
  ...
  track <- filterAndTrim(nop, filts, minQ=3, minLen=500, maxLen=1800, maxN=0, rm.phix=T, maxEE=2, multithread=threads_to_use)
  ...
  # Sequence length after filtering (Corrected)
  print("Analyzing sequence lengths after filtering...")
  lens_filt_start_time <- Sys.time()
  # Check if filtered files exist and are not empty before calculating lengths
  filts_exist <- file.exists(filts)
  if (any(filts_exist)) {
     lens.fn.filtered <- lapply(filts[filts_exist], function(fn) {
       seqs <- tryCatch(getSequences(fn), error = function(e) character(0)) # Handle empty files
       nchar(seqs)
     })
     lens.filtered <- do.call(c, lens.fn.filtered)
     if (length(lens.filtered) > 0) {
       histo_filtered <- hist(lens.filtered, 100, main = paste(run, "filtered"))
       assign(paste(run, "histo_filtered", sep = "_"), histo_filtered)
     } else {
       print(paste("Warning: No sequences found in filtered files for run", run))
       # Optionally assign NULL or an empty plot object
       assign(paste(run, "histo_filtered", sep = "_"), NULL)
     }
  } else {
     print(paste("Warning: No filtered files found for run", run))
     assign(paste(run, "histo_filtered", sep = "_"), NULL)
  }
  lens_filt_end_time <- Sys.time()
  print(paste("Filtered length analysis completed in:", difftime(lens_filt_end_time, lens_filt_start_time, units="mins"), "minutes"))  
  # Dereplicate
  ...
  dd <- dada(drp, err=err, BAND_SIZE=16, multithread=threads_to_use, selfConsist=TRUE)
...
dd <- list()  # Changed from vector() to list()
for(i in runs){
	  dd[[i]] <- get(paste(i, "_dd", sep=""))  # Changed from c() to list indexing with [[]]
}
saveRDS(dd, paste(path.rds, "dd.RDS", sep=""))
...
seqtab.nochim <- removeBimeraDenovo(seqtab, method="consensus", multithread=threads_to_use, verbose=TRUE)
...
taxa <- assignTaxonomy(seqtab.nochim, taxa_database, multithread=threads_to_use, tryRC=TRUE)
...
#ASV97_cluster <- DECIPHER::Clusterize(ASV_dist, method = "complete", processors = nproc, cutoff = 0.03) # cutoff = 0.03 -> 97% OTU 
#ASV98_cluster <- DECIPHER::Clusterize(ASV_dist, method = "complete", processors = nproc, cutoff = 0.02) # cutoff = 0.02 -> 98% OTU 
#ASV99_cluster <- DECIPHER::Clusterize(ASV_dist, method = "complete", processors = nproc, cutoff = 0.01) # cutoff = 0.01 -> 99% OTU
hc <- hclust(as.dist(ASV_dist), method="complete")
...
if(output){
  ...
  for (run in runs) {
    sp <- gsub("microbials", "m", run) # Search pattern
    DAT <- seqtab.nochim[grep(sp, rownames(seqtab.nochim)),]
    # Add error handling to skip empty datasets
    if(nrow(DAT) > 0 && sum(DAT) > 0) {
      tryCatch({
        rarecurve(DAT, step = 20, label=F, main = paste("Rarefaction", run), ylab="ASVs", xlab="Sequencing depth")
      }, error = function(e) {
        # Print error message but continue processing
        print(paste("Error creating rarefaction curve for", run, ":", e$message))
      })
    } else {
      print(paste("Skipping rarefaction curve for", run, "- insufficient data"))
    }
  }
  dev.off()
