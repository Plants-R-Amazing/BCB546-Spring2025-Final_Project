...
rm(list=ls())
#### CHANGE THIS PATHS ####
source <- "/lab/biohpc/students/sadiya/r_analysis_proj/project"
path.in <- "/lab/biohpc/students/sadiya/r_analysis_proj/project/01_demultiplex/out/"
path.primer <- "/lab/biohpc/students/sadiya/r_analysis_proj/project/01_demultiplex/"
path.rds <- "RDS/"
path.out <- "output/"
taxa_database <- "/lab/biohpc/students/sadiya/r_analysis_proj/project/utax_reference_dataset_10.05.2021.fasta"
...
for (run in runs) {
  ...
  # *** MOD: Instead of using nop (files with primers removed), use original file
  # Create directory for filtered files
  dir.create(file.path(path_run, "filtered"), showWarnings=FALSE, recursive=TRUE)
  ...
  # *** MOD: use fns instead of nop as input
  track <- filterAndTrim(fns, filts, minQ=3, minLen=500, maxLen=1800, maxN=0, rm.phix=T, maxEE=2, multithread = nproc)
  ...
  err <- learnErrors(drp, errorEstimationFunction=PacBioErrfun, BAND_SIZE=32, multithread=nproc, qualityType="FastqQuality")
  assign(paste(run, "err", sep="_"), err)
  saveRDS(err, paste(path.rds, run, "_err.RDS", sep=""))
  
  #denoise with inference algorithm
  dd <- dada(drp, err=err, BAND_SIZE=32, multithread=nproc, selfConsist=TRUE)
...
#  }
#}
missing <- setdiff(rownames(taxa), ASV_seq$seq)
if (length(missing) > 0) {
  warning("Some rownames in taxa were not found in ASV_seq")
}

rownames_taxa <- rownames(taxa)
taxa <- as.data.frame(taxa)
taxa$seq <- rownames_taxa
taxa <- left_join(taxa, ASV_seq, by = "seq")
rownames(taxa) <- gsub(">", "", taxa$ASV)
taxa$ASV <- NULL
taxa$seq <- NULL
